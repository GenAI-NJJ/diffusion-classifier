...
def eval_prob_adaptive(unet, latent, text_embeds, scheduler, args, latent_size=64, all_noise=None):
    ...
    for n_samples, n_to_keep in zip(args.n_samples, args.to_keep):
        ...
        pred_errors, pred_latents = calculate_noise(unet, scheduler, latent, all_noise, ts, noise_idxs,
        ...
        for prompt_i in remaining_prmpt_idxs:
            ...
            prompt_pred_latents = pred_latents[mask]
            if prompt_i not in data:
                data[prompt_i] = dict(t=prompt_ts, pred_errors=prompt_pred_errors, pred_latents=prompt_pred_latents)
            else:
                data[prompt_i]['t'] = torch.cat([data[prompt_i]['t'], prompt_ts])
                data[prompt_i]['pred_errors'] = torch.cat([data[prompt_i]['pred_errors'], prompt_pred_errors])
                data[prompt_i]['pred_latents'] = torch.cat([data[prompt_i]['pred_latents'], prompt_pred_latents])

    return data


def calculate_noise(unet, scheduler, latent, all_noise, ts, noise_idxs,
               text_embeds, text_embed_idxs, batch_size=32, dtype='float32', loss='l2'):
    assert len(ts) == len(noise_idxs) == len(text_embed_idxs)
    pred_errors = torch.zeros(len(ts), device='cpu')
    pred_latents = torch.zeros((len(ts), *all_noise.shape[1:]), device='cpu')
    idx = 0
    with torch.inference_mode():
        for _ in tqdm.trange(len(ts) // batch_size + int(len(ts) % batch_size != 0), leave=False):
            ...
            pred_latent = noised_latent - noise_pred * ((1 - scheduler.alphas_cumprod[batch_ts]) ** 0.5).view(-1, 1, 1, 1).to(device)
            
            pred_latents[idx: idx + len(batch_ts)] = pred_latent.detach().cpu()
            
            
            idx += len(batch_ts)
    return pred_errors, pred_latents



def decode_pred_datas(vae, pred_datas, args, img_input):
    
    prompt_idxs = list(pred_datas.keys())
    num_prompts = len(prompt_idxs)
    ts = pred_datas[prompt_idxs[0]]['t']
    num_ts = len(ts)
    
    img_size = args.img_size
    mosaic_w, mosiac_h = num_prompts * img_size, num_ts * img_size
    
    full_mosaic = np.zeros((mosiac_h, mosaic_w, 3), dtype=np.uint8)
    diff_mosaic = np.zeros((mosiac_h, mosaic_w, 3), dtype=np.uint8)
    
    if len(img_input.shape) == 3:
        img_input = img_input.unsqueeze(0)
    
    img_input = img_input.detach().cpu().permute(0, 2, 3, 1).numpy()
    img_input = ((img_input/2+0.5) * 255).round().astype("uint8")
    
    for i, prompt_idx in enumerate(prompt_idxs):
        prompt_data = pred_datas[prompt_idx]
        prompt_ts = prompt_data['t']
        prompt_pred_errors = prompt_data['pred_errors']
        prompt_pred_latents = (prompt_data['pred_latents'] / 0.18215).to(dtype=torch.float16).to(device)
        
        images = vae.decode(prompt_pred_latents).sample.detach().cpu()
        images = (images / 2 + 0.5).clamp(0, 1)
        images = images.detach().cpu().permute(0, 2, 3, 1).numpy()
        images = (images * 255).round().astype("uint8")
        
        diffs = (images - img_input)
        
        for j, t in enumerate(ts):
            
            full_mosaic[j*img_size:(j+1)*img_size, i*img_size:(i+1)*img_size] = images[j]
            diff_mosaic[j*img_size:(j+1)*img_size, i*img_size:(i+1)*img_size] = diffs[j]
    
    diff_mosaic = (diff_mosaic - diff_mosaic.min()) / (diff_mosaic.max() - diff_mosaic.min())
    diff_mosaic = (diff_mosaic * 255).round().astype("uint8")
    
    return full_mosaic, diff_mosaic
...
def main():
    ...
    name = f"{curr_time[2:]}_v{args.version}_{args.n_trials}trials_"
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            x0 *= 0.18215
        pred_datas = eval_prob_adaptive(unet, x0, text_embeddings, scheduler, args, latent_size, all_noise)

        torch.save(dict(pred_datas=pred_datas, prompts=prompts_df), fname)
        
        mosaic_fname = osp.join(run_folder, formatstr.format(i) + '.png')
        diff_fname = osp.join(run_folder, formatstr.format(i) + '_diff.png')
        mosaic, mosaic_diff = decode_pred_datas(vae, pred_datas, args, img_input)
        Image.fromarray(mosaic).save(mosaic_fname)
        Image.fromarray(mosaic_diff).save(diff_fname)
