...
def eval_error(unet, scheduler, latent, all_noise, ts, noise_idxs,
               text_embeds, text_embed_idxs, start_idx=0, batch_size=32, dtype='float32', loss='l2'):
    assert len(ts) == len(noise_idxs) == len(text_embed_idxs)
    pred_errors = torch.zeros(len(ts), device='cpu')
    idx = start_idx
    with torch.inference_mode():
        for _ in tqdm.trange((len(ts)-start_idx) // batch_size + int((len(ts)-start_idx) % batch_size != 0), leave=False):
...
def eval_prob_adaptive_cfg(unet, latent, text_embeds, scheduler, args, latent_size=64, all_noise=None):
    scheduler_config = get_scheduler_config(args)
    T = scheduler_config['num_train_timesteps']
    max_n_samples = max(args.n_samples)
    ...
    data = dict()
    t_evaluated = set()
    remaining_prmpt_idxs = list(range(len(text_embeds)))[:-1]
    base_prompt_idx = len(text_embeds) - 1
    ...
    for n_samples, n_to_keep in zip(args.n_samples, args.to_keep):
        ...
        for prompt_i in remaining_prmpt_idxs:
            for t_idx, t in enumerate(curr_t_to_eval, start=len(t_evaluated)):
                ...
                base_idx += int(args.n_trials)
        
        for t_idx, t in enumerate(curr_t_to_eval, start=len(t_evaluated)):
            ts.extend([t] * args.n_trials)
            noise_idxs.extend(list(range(args.n_trials * t_idx, args.n_trials * (t_idx + 1))))
            text_embed_idxs.extend([base_prompt_idx] * args.n_trials)
                
        t_evaluated.update(curr_t_to_eval)
        
        pred_noise = eval_noise(unet, scheduler, latent, all_noise, ts, noise_idxs,
                                text_embeds, text_embed_idxs, 0, None, args.batch_size, args.dtype, args.loss)
        
        pred_noise_base = pred_noise[base_idx:].repeat(len(remaining_prmpt_idxs), 1, 1, 1)
        pred_noise_cond = pred_noise[:base_idx]
        
        noise = all_noise[noise_idxs].detach().cpu()
        noise_base = noise[base_idx:].repeat(len(remaining_prmpt_idxs), 1, 1, 1)
        noise_cond = noise[:base_idx]
        
        pred_errors_cond = F.mse_loss(noise_cond, pred_noise_cond, reduction='none').mean(dim=(1, 2, 3))
        pred_errors_uncond = F.mse_loss(pred_noise_cond, pred_noise_base, reduction='none').mean(dim=(1, 2, 3))
        
        pred_errors = args.alpha * pred_errors_cond + args.beta * pred_errors_uncond
        
        # match up computed errors to the data
        for prompt_i in remaining_prmpt_idxs:
            mask = torch.tensor(text_embed_idxs[:base_idx]) == prompt_i
            prompt_ts = torch.tensor(ts[:base_idx])[mask]
            prompt_pred_errors = pred_errors[mask]
            if prompt_i not in data:
                data[prompt_i] = dict(t=prompt_ts, pred_errors=prompt_pred_errors)
            else:
...
def eval_noise(unet, scheduler, latent, all_noise, ts, noise_idxs,
               text_embeds, text_embed_idxs, start_idx=0, end_idx=None, 
               batch_size=32, dtype='float32', loss='l2'):
    assert len(ts) == len(noise_idxs) == len(text_embed_idxs)
    pred_noises = torch.zeros((len(ts), *all_noise.shape[1:]), device='cpu')
    idx = start_idx
    end_idx = len(ts) if end_idx is None else end_idx
    
    with torch.inference_mode():
        for _ in tqdm.trange((end_idx-start_idx) // batch_size + int((end_idx-start_idx) % batch_size != 0), leave=False):
            batch_ts = torch.tensor(ts[idx: idx + batch_size])
            noise = all_noise[noise_idxs[idx: idx + batch_size]]
            noised_latent = latent * (scheduler.alphas_cumprod[batch_ts] ** 0.5).view(-1, 1, 1, 1).to(device) + \
                            noise * ((1 - scheduler.alphas_cumprod[batch_ts]) ** 0.5).view(-1, 1, 1, 1).to(device)
            ...
            text_input = text_embeds[text_embed_idxs[idx: idx + batch_size]]
            noise_pred = unet(noised_latent, t_input, encoder_hidden_states=text_input).sample
            pred_noises[idx: idx + len(batch_ts)] = noise_pred.detach().cpu()
            idx += len(batch_ts)
    return pred_noises
...
def main():
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            x0 *= 0.18215
        pred_idx, pred_errors = eval_prob_adaptive_cfg(unet, x0, text_embeddings, scheduler, args, latent_size, all_noise)
